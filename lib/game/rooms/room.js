...
.defines(function(){
    PopUpMenu = ig.Class.extend ({
        ...
        set: function(x, y, type) {
            ...
            this.img = null;
            if(type == 'move') {
                this.img = new ig.Image('lib/game/media/PopUps/moveMenu.png');
            }
            else if(type == 'attack') {
                this.img = new ig.Image('lib/game/media/PopUps/attackMenu.png');
            }
            else if(type == 'loot') {
                this.img = new ig.Image('lib/game/media/PopUps/lootMenu.png');
            }
            else {
                this.img = new ig.Image('lib/game/media/PopUps/noActionMenu.png');
            }
            this.texture = null;
    ...
    Tile = ig.Class.extend ({
        ...
        setImage : function(imgName, type) {
            this.img = new ig.Image(imgName);
            this.texture = glLoadTexture(this.img.data);
            this.occupied = true;
            this.hasOptions = true;
            this.menu.set(this.x+65, this.y+40, type);
        },
        draw: function(lightIntense) {
            if(this.img != null) glDraw(this.img.width, this.img.height, this.x, this.y, lightIntense, this.texture);
            if(this.selected) {
                this.menu.draw(this.x+65, this.y+40, lightIntense);
            }
        },
        ...
        optionPopUp : function(player) {
            if(this.menu.isClickTop()) {
                if(this.menu.type == 'move') {
                    player.x = this.x;
                    player.y = this.y; 
                }
                else if (this.menu.type == 'attack') {
                    console.log('attack');
                }
                else if (this.menu.type == 'loot') {
                    console.log('loot');
                }
                else console.log("what did you click on?!?!?!?!?");
            }
            else if (this.menu.isClickBottom()) {
	...
	NewRoom = ig.Class.extend({
        ...
        init: function() {
            ...
            this.textures.push(glGenerateEmissive(this.emissive.data));
        ...
        setTile: function(tileIDnum, imageFileName, type) {
            //example : this.tiles[0].setImage('image.png');
            this.tiles[tileIDnum].setImage(imageFileName, type);
        },
    ...
    Tyche = NewRoom.extend({
        ...
        init: function(){
            ...
            this.initiateTiles(this.roomX, this.roomY, this.tileGridWidth, this.tileGridHeight, this.tileHeightOffset);
            //set tiles here
            //example: this.setTile(97, 'lib/game/media/door.png', 'loot');
            this.setTile(32, 'lib/game/media/door.png', 'loot');
            this.setTile(45, 'lib/game/media/door.png', 'attack');
            this.setTile(25, 'lib/game/media/door.png', 'move');

            //create the possible room doors tiles (currently positions set according to grid of 10x10)
            this.doors = [];
            this.setDoors(50, 5, 59, 95);
