...
.defines(function () {
    ...
    LogoScreen = ig.Game.extend({
        //declare variables
        font: new ig.Font('media/04b03.font.png'),
        background: new ig.Image('lib/game/media/lasercat.png'),
        name: new ig.Image('lib/game/media/BrandName.png'),
        textures: new Array(), 
    ...
    StartScreen = ig.Game.extend({
        //declare variables
        background: new ig.Image('lib/game/media/logos.jpg'),
    ...
    CreateScreen = ig.Game.extend({       
        label: new ig.Image('lib/game/media/CreateGame.png'),
        textures: new Array(), 
    ...
    JoinScreen = ig.Game.extend({ 
        label: new ig.Image('lib/game/media/JoinGame.png'),
        textures: new Array(), 
    ...
    LobbyScreen = ig.Game.extend({
        label: new ig.Image('lib/game/media/Lobby.png'),
        textures: new Array(), 
    ...
    OptionsScreen = ig.Game.extend({
        label: new ig.Image('lib/game/media/Options.png'),
        textures: new Array(), 
    ...
    GameScreen = ig.Game.extend({
        staticUI : new ig.Image('lib/game/media/gameUIClosed.png'),        
        textures: new Array(), 
        ...
        update: function () {
            ...
            if(ig.input.pressed('leftMouse')) {
                ...
                //if on a tile in room, check if it's occupiable, then if it's not, move the player there
                    ...
                    for(var i = 0; i < 100; i++){
                        if(this.currentRoom.tiles[i].selected && this.currentRoom.tiles[i].menu.isOnPopUp()) {
                            this.currentRoom.tiles[i].optionPopUp(this.player1);
                        }
                        else if(this.currentRoom.tiles[i].isMouseOnTile()) {      //bottom left
                            if(this.currentRoom.tiles[i].hasOptions) {
                                this.currentRoom.tiles[i].selected = true;
                            }
                            else {
                        ...
                        else if(this.currentRoom.tiles[i].selected) this.currentRoom.tiles[i].selected = false;
                    }
                }
            }
        ...
        draw: function () {
            ...
            glDraw(this.staticUI.width, this.staticUI.height, cntrX, cntrY, 1.0, this.textures[0]);
        }
    });

    //screen to display post-game stats
    StatsScreen = ig.Game.extend({
        label: new ig.Image('lib/game/media/Stats.png'),
        textures: new Array(), 
